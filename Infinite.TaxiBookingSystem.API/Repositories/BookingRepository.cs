...
namespace Infinite.TaxiBookingSystem.API.Repositories
{
    public class BookingRepository : IRepository<Booking>,IGetRepository<BookingDto>, IBookingRepository
    {
        ...
        public async Task<IEnumerable<TaxiType>> GetTaxiTypes()
        {
            var taxitypes = await _context.TaxiTypes.ToListAsync();
            return taxitypes;
        }
        public async Task<IEnumerable<TaxiModel>> GetTaxiModels()
        {
            var taximodel = await _context.TaxiModels.ToListAsync();
            return taximodel;
        }
        ...
        //    {
        //        var bookings = await _context.Bookings.Include(x => x.Taxi).Where(x => x.Taxi.TaxiModelId.Contains(taxiModel)).ToListAsync();
        //        return bookings;
        //    }
