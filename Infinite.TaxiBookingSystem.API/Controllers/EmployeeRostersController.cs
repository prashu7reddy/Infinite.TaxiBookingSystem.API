...
namespace Infinite.TaxiBookingSystem.API.Controllers
{
    ...
    public class EmployeeRostersController : ControllerBase
    {
        ...
        private readonly IEmployeeRosterRepository _employeeRosterRepository;
        private readonly IGetRepository<EmployeeRosterDto> _employeeRosterDtoRepository;

        public EmployeeRostersController(IRepository<EmployeeRoster> repository,IEmployeeRosterRepository employeeRosterRepository,IGetRepository<EmployeeRosterDto> employeeRosterDtoRepository)
        {
            _repository = repository;
            _employeeRosterRepository = employeeRosterRepository;
        ...
        {
            var result = await _employeeRosterRepository.SearchByEmp(employeeName);
            if (result != null)
            {
                return Ok(result);
            }
